...
export default function AuthorInfo({ name, linkedinUrl, githubUrl, portfolioUrl }: AuthorInfoProps) {
  return (
    <div className="fixed top-2 left-2 z-50 flex items-center gap-2 bg-background/80 backdrop-blur-sm p-2 rounded-lg shadow-sm border">
      <span className="text-sm font-medium hidden md:inline-block">Created by {name}</span>
      <span className="text-sm font-medium md:hidden">By {name}</span>
